...
class SineCosineClass
{
    ...
    SineCosineClass(SCAParamClass param)
    {
        ...
        //allocate memory for population citerion class
        V = new PopulationCriterion(param.population_size, param.dimension);
    ...
    double execute(int function_id){
        ...
        for (int tvar = 0; tvar<param.iter_max; tvar++){
            ...
            population->evaluate_costval(function_id);//find fitness of each item of population
            population->calculate_min_costval();// find minimum fitness of population
            if(population->get_min_costval() < global_optimal_cost){// obtain global minumum
                global_optimal_cost = population->get_min_costval();
                global_optimal_set_data(population->get_min_costval_data());
            }
