    ...
    double execute(int function_id){
        ...
        for (int tvar = 0; tvar<param.iter_max; tvar++){
            ...
            for(int j=0;j<param.dimension;j++){
                ...
                if(j==0){
                    max = param.limits.lower_x;
                    min = param.limits.upper_x;
                }
                if(j==1){
                    max = param.limits.lower_y;
                    min = param.limits.upper_y;
                }
                if(j==2){
                    max = param.limits.lower_z;
                    min = param.limits.upper_z;
                }
                ...
                if(j==0)
                    stay_in_range(xvarl,param.limits.lower_x, param.limits.upper_x);
                if(j==1)
                    stay_in_range(xvarl,param.limits.lower_y, param.limits.upper_y);
                if(j==2)
                    stay_in_range(xvarl,param.limits.lower_z, param.limits.upper_z);
