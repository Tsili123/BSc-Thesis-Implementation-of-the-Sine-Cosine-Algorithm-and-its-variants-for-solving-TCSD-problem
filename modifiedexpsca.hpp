...
class SineCosineClass
{
    ...
    double execute(int function_id){
        ...
        for (int tvar = 0; tvar<param.iter_max; tvar++){
            ...
            double omega_max = OMG_MAX;
            double omega_min = OMG_MIN;
            ...
            for (int i = 0; i<param.population_size; i++){
                ...
                for(int j=0; j<param.dimension; j++){
                    ...
                    if (r4 < 0.5) //sine
                        new_xvar = omega * xvar + r1*sin(r2)*fabs(r3*global_optimal_data[j]-xvar);
                    else //cosine
                        new_xvar = omega * xvar + r1*cos(r2)*fabs(r3*global_optimal_data[j]-xvar);
